...
function FilmGenre({ data }) {
  ...
  return (
    <div className="flex flex-col w-full">
      ...
      <div className="bg-[#373646] py-6 px-4 mt-6 max-[478px]:bg-transparent max-[478px]:px-0 rounded-xl">
        <div className="grid grid-cols-3 grid-rows-2 gap-x-4 gap-y-3 w-full max-[478px]:flex max-[478px]:flex-wrap max-[478px]:gap-2">
          {data &&
            (showAll ? data : data.slice(0, 24)).map((item, index) => {
              ...
              return (
                <Link
                  to={`/film/category/${item.toLowerCase()}`}
                  key={index}
                  className="rounded-[4px] py-2 px-3 hover:bg-[#555462] hover:cursor-pointer max-[478px]:bg-[#373646] max-[478px]:py-[6px]"
                  style={{ color: textColor }}
                >
                  <div className="overflow-hidden text-left text-ellipsis text-nowrap font-bold text-[14px]">
...
export default React.memo(FilmGenre);
