...
const CategoryCard = React.memo(
  ({
    ...
    const getItemsToRender = useCallback(() => {
      if (categoryPage) {
        const firstRow =
          window.innerWidth > 758 && safeData.length > 4 ? safeData.slice(0, 4) : [];
        const remainingItems =
          window.innerWidth > 758 && safeData.length > 4
            ? safeData.slice(4)
            : safeData;
        return { firstRow, remainingItems };
      } else {
        return { firstRow: [], remainingItems: safeData };
      }
    }, [categoryPage, safeData]);
              ...
              {itemsToRender.firstRow.map((item, index) => (
                ...
                >
                  ...
                  <div className="flex items-center gap-x-2 w-full mt-2 overflow-hidden">
                    <div className="text-gray-400 text-[14px] text-nowrap overflow-hidden text-ellipsis flex items-center gap-x-1 capitalize">
                      {item.tvInfo?.showType?.toLowerCase() === "manga" ? <span className="px-1 rounded text-[10px] font-bold border border-gray-600">JP</span> : item.tvInfo?.showType?.toLowerCase() === "manhwa" ? <span className="px-1 rounded text-[10px] font-bold border border-gray-600">KR</span> : item.tvInfo?.showType?.toLowerCase() === "manhua" ? <span className="px-1 rounded text-[10px] font-bold border border-gray-600">CN</span> : ""} 
                      {item.tvInfo?.showType?.split(" ").shift()}
                    </div>
                    <div className="dot"></div>
                    <div className="text-gray-400 text-[14px] text-nowrap overflow-hidden text-ellipsis flex items-center gap-x-1">
                      <FontAwesomeIcon icon={faStar} className="text-[#FFD700] text-[12px]" />
                      {item.tvInfo?.rating || "N/A"}
                    </div>
                  </div>
                </div>
              ))}
