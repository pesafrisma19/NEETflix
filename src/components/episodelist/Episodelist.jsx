        ...
        >
          {totalEpisodes > 30
            ? episodes
              ...
              .map((item, index) => {
                const episodeNumber = item?.episode_no || String(item?.id).match(/ep=(\d+)/)?.[1];
                const isActive =
                ...
                return (
                  <div
                    ...
                    className={`flex items-center justify-center rounded-[3px] h-[30px] text-[13.5px] font-medium cursor-pointer group ${item?.filler
                        ? isActive
                          ? "bg-[#ffbade]"
                          : "bg-[#252528] md:hover:bg-[#ffbade] text-white"
                        : isActive
                          ? "bg-[#ffbade] text-[#252528]"
                          : "bg-[#252528] md:hover:bg-[#ffbade]"
                      } ${isSearched ? "border-2 border-red-500" : ""}`}
                    onClick={() => {
                  ...
                  >
                    <p
                      className={`group-hover:text-black ${item?.filler
            ...
            : episodes?.map((item, index) => {
              ...
              return (
                <div
                  key={item?.id}
                  ref={isActive ? activeEpisodeRef : null}
                  className={`w-full pl-5 pr-2 py-3 flex items-center justify-start gap-x-8 cursor-pointer ${(index + 1) % 2 && !isActive
                      ? "bg-[#201F2D] text-gray-400"
                      : "bg-none"
                    } group md:hover:bg-[#2B2A42] ${isActive ? "text-[#ffbade] bg-[#2B2A42]" : ""
                ...
                >
                  ...
                  <div className="w-full flex items-center justify-between gap-x-[5px]">
                    <h1 className="line-clamp-1 text-[15px] font-light group-hover:text-[#ffbade]">
                      {item?.tmdb?.title || `Episode ${episodeNumber}`}
                    </h1>
                    {isActive && (
                      <FontAwesomeIcon
                        icon={faCirclePlay}
